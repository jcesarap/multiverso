...
contextBridge.exposeInMainWorld("api", {
  ...
  setGitUsername: (username) =>
    ipcRenderer.invoke("set-git-username", username),
  ...
  setWorkingDirectory: (dirPath) =>
    ipcRenderer.invoke("set-working-directory", dirPath),
  printDir: () => ipcRenderer.invoke("printDir"),
  switchBranchOnClick: (branchName) =>
    ipcRenderer.invoke("switch-branch", branchName),
  addBranch: (branchTitle) => ipcRenderer.invoke("add-branch", branchTitle),
  renameBranch: (newBranchTitle) =>
    ipcRenderer.invoke("rename-branch", newBranchTitle),
  deleteBranch: (currentBranch) =>
    ipcRenderer.invoke("delete-branch", currentBranch),
  ensureGitSetup: () => ipcRenderer.invoke("ensure-git-setup"),
  showDialog: (message) => ipcRenderer.invoke("show-dialog", message),
  gitCommit: (commitTitle, commitBody) =>
    ipcRenderer.invoke("git-commit", commitTitle, commitBody),
  loadCommits: () => ipcRenderer.invoke("load-commits"),
  createPastBranch: (previousBranchName, hash, selectedCommitDate) =>
    ipcRenderer.invoke(
      "create-past-branch",
      previousBranchName,
      hash,
  ...
  loadRecentPaths: () => ipcRenderer.invoke("load-recent-paths"),
});
